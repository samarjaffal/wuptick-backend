...
const { randomColor } = require('../../functions/general');
...
module.exports = {
    ...
    inactivateAccountByLoggedUser: async (_, { }, context) => {
        try {
            let user, id;
            if (!context.isAuth) {
                return null;
            ...
            await Auth.revokeRFTokensForUser(user._id);
            await Auth.inactivateAccount(user._id);
            return true;
        } catch (error) {
            console.error(error);
            return false;
