...
const notificationLoader = new DataLoader(async (notificationIds) => {
    let notifications;
    try {
        notifications = await mongoHelper
            .getAllDocuments('notifications', notificationIds)
            .then((notifications) =>
                notificationIds.map((id) =>
                    notifications.find((notification) =>
                        notification._id.equals(id)
                    )
                )
            );
    } catch (error) {
