...
const { userLoader, teamLoader, roleLoader } = require('../db/dataLoaders');
    ...
    Project: {
        owner: async ({ owner }) => {
            ...
            try {
                ...
                userData = await userLoader.load(owner);
            } catch (error) {
                console.error(error);
            }
            return userData;
        ...
        members: async ({ members }) => {
            ...
            try {
                ...
                const filteredUsers = usersData.filter((user) => user.status === 'active');

                rolesData = await roleLoader.loadMany(roleIds);
                /* teamsData = await mongoHelper.getAllDocuments('teams', teamIds); */
                teamsData = await teamLoader.loadMany(teamIds);

                data = members.map((member) => {
                    let user = filteredUsers.find((data) =>
